...
export default function SignupPage() {
  ...
  const handleSignup = async (event: FormEvent) => {
    ...
    // Supabase will typically send a verification email (depending on your settings)
    setInfo(
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4">
      <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900/80 p-8 shadow-xl">
        <h1 className="text-2xl font-semibold text-slate-50 mb-2">
          Create your Work Zone OS account
        </h1>
        <p className="text-sm text-slate-400 mb-6">
          Sign up with your email to get access to the dashboard.
        </p>

        {info && (
          <div className="mb-4 rounded-lg border border-emerald-500/50 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-200">
            {info}
          </div>
        )}
